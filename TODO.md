...
| **タグ体系** | `Sketch / Articles / Event・Live / Hobby / Other` の5タブ構成。`Articles` は旧 `tech` カテゴリに専用タブを与えたもの（enum値も `tech` → `article` にリネーム）。加えて `portfolio` をenumに追加（`sketch` とは別枠）。タブ名・ルート・`sketch` との使い分けは未定（9章）。 |
...
- `portfolio` を新規カテゴリとして追加（`sketch` とは別枠）。サイドバー表示ラベル・一覧ルート・`sketch` との使い分けの基準は未定のため要確認事項（9章）に記載。
- 上記スキーマ変更（`category` リネーム＋`portfolio` 追加、`noindex` 追加）は `src/content.config.ts` に実装済み。
...
| `portfolio` | （未定） | （未定、仮 `/blog/portfolio`） |
...
- [ ] `portfolio` カテゴリのサイドバー表示ラベル・一覧ルート（2.2）と、`sketch` との使い分けの基準。
