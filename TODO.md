...
- [ ] 記事本文（`.prose`/`.content`）のフォントを明朝体系（serif）にする。現状 `global.css` の `.title-font`/`.content-title-font` はタイトル用にGeorgia/serifを指定済みだが、本文側には未指定。使用フォントは **Zen Old Mincho**（Google Fonts、OFLライセンス）に決定。配信方式は `@fontsource` 経由でのセルフホストを予定（9章）。
