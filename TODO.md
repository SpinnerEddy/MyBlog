...
- `src/components/LeftPanel.astro`（旧`RightPanel.astro`、3カラムの左カラムに配置するため改名）は空プレースホルダー。カレンダー・X埋め込みは未着手。
...
### 7.1 レイアウト全体
Order 12（Home）・11（Blogカテゴリ別一覧）を待たず先行実装。以下の3点は完了。
- [x] `BaseLayout.astro`: コメントアウトされている3カラムグリッドを有効化し、`Sidebar` と `LeftPanel`（旧`RightPanel`）を配置。
- [x] `Sidebar.astro`: `navItems` を `Home / About / Blog(サブメニュー: All/Sketch/Portfolio/Articles/Event・Live/Hobby/Other) / PlayGround` に更新。データ構造を `{ href, label, children? }` に拡張し、現在地判定でアクティブ表示（`docs/design-notes/sidebar-submenu.md`通り、`aria-current`ベース）。Blogサブメニューは`All`（`/blog`）のみ実リンク、他6項目（Sketch/Portfolio/Articles/Event・Live/Hobby/Other）はカテゴリ別一覧ページ未実装（Order 11）のため`<span aria-disabled>`のグレーアウト表示に留めた。
- [x] `LeftPanel.astro`（旧`RightPanel.astro`）: 上部に `Calendar.astro`、下部に `XTimeline.astro` を配置。→ 当初は両方未実装のため空プレースホルダーだったが、Order 14を前倒しして実装済み（7.7, 7.8参照）。
- [x] `PlayGround`ページ（Order 13）を空ページとして先行作成（`src/pages/playground.astro`）。Sidebarのリンク切れ防止のため。
- [x] 視認性改善: `Sidebar.astro`のトップレベル項目間に`divide-y`区切り線、`BaseLayout.astro`のSidebar/LeftPanelそれぞれに`border-r`/`border-l`を追加し、3カラムの境界を分かりやすくした。
- [x] 装飾強化: `Sidebar.astro`のnavに上部余白（`pt-8`）を追加、トップレベル項目に`title-font`（Georgia）と`text-base`を適用、アクティブ項目に`bg-zinc-100`のピル背景を追加。
- [x] 配置入れ替え・改名: `BaseLayout.astro`の3カラムを`Sidebar`が右（240px）・`RightPanel`が左（300px）になるよう変更したうえで、左に来た`RightPanel.astro`を`LeftPanel.astro`に改名（`grid-cols-[300px_1fr_240px]`）。
- [x] 余白調整: Sidebar/LeftPanelの境界線側パディングを`pr-6`/`pl-6`→`pr-3`/`pl-3`に縮小し、`gap-6`との二重の余白でコンテンツ領域が窮屈に見える問題を解消。
...
- [x] `PortfolioAboutMeText.astro` の内容を移設。→ Order 8（3カラムレイアウト）を待たず先行実装。`BaseLayout.astro`に`<slot />`を導入（`docs/design-notes/3-column-layout.md`で計画済みの前提変更）し、`index.astro`は`<PortfolioPageLayout />`を子要素として渡す形に変更。`about.astro`は`<PortfolioAboutMeText />`のみを渡す。ホーム（`index.astro`）の表示は変更なし。
- [x] Aboutの本文（自己紹介文・使用言語など）は運用中の変更頻度が高いと想定されるため、`PortfolioAboutMeText.astro`への直書きをやめ、Content Collections化。`content.config.ts`に`about`コレクション（`name`/`icon`のフロントマター + Markdown本文）を追加し、`src/content/about/index.md`で管理。`PortfolioAboutMeText.astro`は`getEntry('about', 'index')`→`render()`で読み込むだけに変更。ホーム/Aboutどちらも表示内容は変更なし（ビルド・実ブラウザで確認済み）。
...
Order 8を待たず先行実装。`LeftPanel.astro`上部に配置済み。
- [x] `getCollection('blog')` の `pubDate` を日付別に集計し、週×曜日のCSS Gridヒートマップを生成。→ **デザイン変更**: ヒートマップ版は52週分の横幅がLeftPanel(300px)に収まらずコンテンツ領域へ侵食するバグがあり、かつ「通常の年月日が分かるカレンダー」という要望と異なったため、日曜始まりの月間カレンダー（今月のみ表示、投稿日にマーク、今日を枠線でハイライト）に全面書き換え。`pubDate`の日付キー生成は`toISOString()`（UTC変換でタイムゾーンずれが起きる）ではなく、`getFullYear()`/`getMonth()`/`getDate()`のローカル日付ベースに変更。
- [x] 月送りナビゲーション（前月/翌月ボタン）を追加。投稿がある月データをJSONとしてクライアントへ埋め込み、`<script>`側でグリッドを再生成する形で実装。最古の投稿月まで遡ると前ボタンが、実際の今月に達すると次ボタんが自動的に無効化される。
- [x] 装飾強化: `Sidebar`と対称になるよう`LeftPanel`全体に`pt-8`を追加、月見出しに`title-font`（Georgia）+太字を適用、前月/翌月ボタンを追加。
- [x] バグ修正3件: (1) `rgb(var(--accent) / 15%)`が`--accent`のカンマ区切り定義と衝突し無効なCSSになっていたため、投稿日ハイライトの背景色をリテラル値（`#eef1ff`）に変更。(2) クライアントJSで`document.createElement`した日付セルにAstroのスコープ付きCSS属性（`data-astro-cid-*`）が付与されずスタイルが当たらなかったため、静的要素から属性名を取得して複製するよう修正。(3) 月見出し（`title-font`=Georgia）の数字がオールドスタイル数字（3/4/5/7/9等がベースラインより下にずれるデザイン）になっており「年」「月」と高さが揃わなかったため、`.calendar-title`に`font-variant-numeric: lining-nums;`を追加してライニング数字に統一。

### 7.8 X タイムライン（新規: `src/components/XTimeline.astro`）
Order 8を待たず先行実装したが、実ブラウザで安定して表示されないため**一旦`LeftPanel.astro`から外し保留**。コンポーネント自体は残してあり、`LeftPanel.astro`のimportをコメントインすれば再度有効化できる。
- [x] 公式 `twitter-timeline` 埋め込み + `widgets.js` を、IntersectionObserverで遅延読み込み。→ `twttr.widgets.createTimeline()`によるiframe生成まで動作確認済み。実ブラウザで表示されない報告あり（X側APIのレート制限/広告ブロッカー等が原因の可能性）のため、8秒でタイムアウトし`https://x.com/{screenName}`へのリンクにフォールバックする処理を追加（`script.onerror`ハンドリングも追加）。
- [x] 実装方式の見直し: `twttr.widgets.createTimeline()`（JS APIでの動的生成）から、X Publish（publish.twitter.com）が生成する公式の静的マークアップ（`<a class="twitter-timeline">` + `widgets.js`）と同じ形に変更。ただしフルページスキャンではなく、遅延読み込みを維持するため`twttr.widgets.load(対象要素)`でその要素だけをスキャン対象にする方式にした。あわせて、フォールバック判定を「iframeが挿入されたか」ではなく「iframeが実際に高さを持ったか」に修正（X側のデータ取得が失敗すると中身が空のiframeだけ挿入されるケースがあり、旧判定だと見逃していた）。→ この修正後も実ブラウザで改善が見られなかったため、根本的にX側の埋め込みタイムラインAPIの制約・不安定さが原因と判断。
- [ ] `LeftPanel.astro`から`<XTimeline />`の呼び出しを外し、`Calendar`のみ表示する状態に戻した。再度有効化するか、フォールバックリンクのみを常時表示する方式に変更するかは今後判断する。
...
- [ ] 3カラムレイアウト（`Sidebar`/`LeftPanel`、7.1）・記事本文のprose表示（4.1）・ライブデモcanvas（5.2, 5.3）・カレンダーヒートマップ（7.7）は特に幅依存のCSSが多いため、モバイル幅でのブレイクポイント/折り返し挙動を重点的に確認する。
- [ ] 確認して見つかった崩れは都度このTODOか各タスクのチェック項目に追記し、対応してから完了とする。

### 7.10 サイトへのインタラクション追加（ページ遷移アニメーション）
Order 8（Sidebar/3カラム）実装中に構想。今回はスコープ外、着手順序20番目として別タスク化（Notion「20. サイトへのインタラクション追加」）。
- [ ] Astroの View Transitions API（`astro:transitions`の`<ClientRouter />`）を導入。
- [ ] Sidebarのアクティブ項目のピル背景に`transition:name`を付与し、ページ遷移時（例: Home→About）にピルの位置がイージングでシームレスに移動するようにする。
- [ ] メインコンテンツ（`<main>`）に`transition:animate`でフェードアウト/フェードインの遷移を付与。「上からフェードイン」等の独自の動きにしたい場合はカスタムキーフレームを検討。
- [ ] `BlogPost.astro`は`BaseLayout`を経由しない独自レイアウトのため、記事詳細ページにも同じ遷移を効かせる場合は`<ClientRouter />`の追加配線が別途必要。
- [ ] Safari/FirefoxはView Transitions APIの対応がChromiumより新しい・限定的。非対応ブラウザでは通常の即時遷移にフォールバックすることを確認する。
...
- [x] X埋め込みの対象アカウント（`@EKey2210` で確定か）。→ `@EKey2210`で確定。
...
20. サイトへのインタラクション追加（ページ遷移アニメーション、7.10）。8番のタスク中に構想、別タスク化。
