...
- [ ] 記事本文（`.prose`/`.content`）のフォントを明朝体系（serif）にする。現状 `global.css` の `.title-font`/`.content-title-font` はタイトル用にGeorgia/serifを指定済みだが、本文側には未指定。使用する具体的なフォント（Webフォント名、CDN配信 or セルフホスト）は未定（9章）。
...
- [ ] 見出し(h1〜h6)・段落・リスト・引用のスタイリングは4.1のtypography対応でカバーする。
- [ ] リンク（URL遷移）は標準Markdownの `[text](url)` でそのまま対応可能（追加実装不要）。
- [ ] 文字色変更・下線・マーカーなど、Markdown標準にない装飾の実現方法を検討する。候補: (a) MDX記事内でインラインHTML/JSX（例: `<span style="color:red">`）を直書きする、(b) `<ColorText color="...">` のような専用コンポーネントを用意する。`.md`（非MDX）ファイルで生HTMLがどこまで通るかも合わせて確認（実装前に `astro-tailwind-research` SKILLで調査）。

### 4.7 メディア埋め込み（画像・動画・音声・動画リンク）
- [ ] 画像: 標準Markdown画像記法（`![alt](./photo.png)`）、記事フォルダにco-locate（2.3参照）。追加実装不要。
- [ ] 自己ホストの動画ファイル: `<video controls src="./movie.mp4" />` をMDX本文に直接記述（5.6の「動画」欄と同じ考え方）。
- [ ] 音声ファイル: `<audio controls src="./sound.mp3" />` をMDX本文に直接記述。
- [ ] 外部動画リンク（YouTube等）: `EmbedPlate` + `embedType`/`embedId` frontmatterで埋め込み（5.4）。
...
- [ ] 本文フォント（明朝体系）に何を使うか（Webフォント名、CDN配信 or セルフホスト）（4.1）。
...
4. `BlogPost.astro` の `.prose` リネーム → Typography導入 → TOC対応、Mermaid・装飾・メディア埋め込み対応（4.1, 4.2, 4.5, 4.6, 4.7）
